...
public class CartWriteServiceImpl implements CartWriteService {
    ...
    public ResponseEntity<CommandResult> addProductToCart(Long productId) {
        ...
        Product product = productRepository.findById(productId)
                .orElseThrow(() -> new AbstractPlatformException("error.product.id.not.found", "Product not available", 404));
        ...
        CartItem cartItem = cartItemRepository.findByProductId(productId)
                .orElseThrow(() -> new AbstractPlatformException("Product with id " + productId + "not found", "Product does not exist", 404));

        if(cartItem != null) {
            increaseQuantity(productId);
            return ResponseEntity.ok(new CommandResultBuilder()
        ...
        return ResponseEntity.ok(new CommandResultBuilder().response("Item added to cart").entityId(cart.getId()).build());
    ...
    public ResponseEntity<CommandResult> removeProductFromCart(Long itemId) {
        ...
        return ResponseEntity.ok(new CommandResultBuilder().response("Item deleted from cart")
                .entityId(cart.getId()).resourceId(String.valueOf(cartItem.getId())).build());
    }
    ...
    public ResponseEntity<CommandResult> clearCart(Long cartId) {
        ...
        return ResponseEntity.ok(new CommandResultBuilder().response("Item deleted from cart")
                .entityId(cart.getId()).resourceId(String.valueOf(cart.getId())).build());
    }
    ...
    public ResponseEntity<CommandResult> increaseQuantity(Long productId) {
        ...
        Product product = this.productRepository.findById(productId)
                .orElseThrow(() -> new AbstractPlatformException("Product with id " + productId + " not found", "Not found", 404));
        ...
        for (CartItem item : cartItems) {
            if(Objects.equals(item.getProduct().getId(), savedCartItem.getProduct().getId())) {
                ...
                savedCartItem.setSubTotal(item.getUnitPrice() .multiply(new BigDecimal(savedCartItem.getOrderQuantity())));
