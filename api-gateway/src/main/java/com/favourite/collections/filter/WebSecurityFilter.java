...
public class WebSecurityFilter implements WebFilter {
	...
	public Mono<Void> filter(ServerWebExchange exchange, WebFilterChain chain) {
		...
		boolean isPermitted = routeValidator.getPathVariables(exchange.getRequest());
		...
		if (BooleanUtils.isFalse(isPermitted)) {
			...
			String authHeader = Objects
					.requireNonNull(exchange.getRequest().getHeaders()
			...
			ServerHttpRequest mutateRequest = exchange.getRequest().mutate()
					.header("user-token", userEmail).build();
			ServerWebExchange mutateServerWebExchange = exchange.mutate()
